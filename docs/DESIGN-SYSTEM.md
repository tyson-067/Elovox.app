...
**Admin, email, and the model contracts.** The last three untested surfaces:

| File | The bug it prevents |
|---|---|
| `admin-routes.test.ts` (80) | The console fails **closed**. An unset or empty `ADMIN_EMAILS` grants nobody (a deploy that forgets the var must lock the console, not read an empty list as "no restrictions"); an *unverified* account holding an operator's address is refused (Firebase will mint a token for anyone who registers `ops@…`); the match is whole-address, so `ops@elovox.app.attacker.example` is not an operator. Every route answers a bare **404**, never 403 — including when the service account is missing or the rate limiter would fire, because those branches once ran *ahead* of the admin check and let an anonymous visitor distinguish a real route from a nonexistent one. Denied callers cause **no** side effect. Plus the mutating routes: `coins` never creates the `score/progress` doc (its absence is a signal three other systems read), `comp` writes nothing the webhook owns, `moderation` cannot strike an operator, and every audit `actor` comes from the verified token rather than the request body. |
| `email-guards.test.ts` (73) | Unsubscribe-token forgery, suppression, idempotency, the daily/monthly budget, and what each `SendOutcome` means. Found three real bugs — see below. |
| `model-contracts.test.ts` | What *our* code does when a vendor misbehaves, against recorded fixtures and no network. The Gemini fallback chain and its cooldown, the per-attempt timeout, a valid-JSON-wrong-shape response and an HTML error page both degrading rather than throwing. For AssemblyAI: a non-ok poll fails in **one** step rather than being politely polled 40 more times (a rotated key returns 401, which used to parse as `{}`), and only an `AAI_INPUT_ERROR` detail becomes an `AudioInputError` — that distinction decides whether the user is told to re-record or told we failed. |

**Three bugs these found, and what happened to them:**

1. **A complaint could be downgraded to a plain unsubscribe.** `applyUnsubscribe`'s unscoped branch called `suppress()` straight through, and `suppress()` merges — so `reason` was overwritten. A plain unsubscribe blocks only *optional* mail, so an address that had pressed the spam button went back to receiving security and billing mail. `writePrefs` already guarded exactly this; the unscoped branch did not. **Fixed.**
2. **`release()` credited the day it ran on.** A send reserved at 23:59 that failed at 00:00 burned a message of today and credited one to tomorrow, whose counter went negative — so tomorrow's *effective* cap rose above Resend's real 100 and message 101 was refused by the provider. A dropped email, not a delayed one. **Fixed** by threading the reservation's timestamp through `send.ts` and `audience.ts`.
3. **The optional categories can still starve a security notice.** `CATEGORY` shares are per-category *ceilings*, not reservations, and the optional ones sum to 110% of a 100-message day (lifecycle 0.6 + marketing 0.5). `/api/cron/email` runs the lifecycle jobs *and* the tips drip in one 09:00 UTC invocation, so on a busy day optional mail can consume the entire allowance before anyone is awake — and every `security` and `billing` message for the next fifteen hours is dropped. This contradicts the module's own stated guarantee. **Not fixed:** closing it means capping the optional categories collectively or floor-reserving for the required ones, which changes *which mail gets dropped* when the day is tight. That is a product decision, so it is pinned by a test named `BUG: two optional runs on one day CAN take all 100 and starve security` and surfaced rather than quietly changed.
