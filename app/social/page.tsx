...
function SocialScreen() {
  ...
  return (
    <div className="py-10 md:py-16">
      {/* App shape: one calm grouped list, same skills, same lock rule as
          the cards below. Renders nothing in a browser. */}
      <NativeLibraryList
        sections={[
          {
            items: SOCIAL_SKILLS.map((s) => ({
              title: s.name,
              blurb: s.description,
              locked: plan === "free",
              href: isPremium ? `/practice?social=${s.id}` : undefined,
            })),
          },
        ]}
      />
